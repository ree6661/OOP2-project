...
public class FirmaController extends ControllerParent {
	...
	public void addOffice() throws SQLException {
		...
		String cityName = this.city.getPromptText(), address = this.address.getText();
		...
		String sql = "select * from office where id_company='" + this.company.getId() + "' and id_city='" + cityIndex
				+ "'and address='" + address + "'";
	...
	private void changeOffice() throws SQLException {
		...
		if (this.office == null || this.offices.getPromptText().isBlank() || this.companies.getPromptText().isBlank()) {
	...
	private void addCourier() throws SQLException {
		...
		String name = this.courierTextField.getText(), phone = this.phone.getText(), password = this.password.getText();
