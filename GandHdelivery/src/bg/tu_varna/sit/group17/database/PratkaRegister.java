...
public final class PratkaRegister {
	...
	private void courierNotification(Button notificationBell) throws SQLException {
		try {
	...
	private boolean invalidFields(double price, String phoneSender, String phoneReceiver, String address,
		...
		try {
			if (!Valid.order(phoneSender, phoneReceiver, this.receiveDate.getValue().toString(),
				...
				throw new NullPointerException("Невалиден телефон на клиент");
			}

			if (!sendToAddress && officeSender.getSelectionModel().getSelectedItem()
					.equals(officeReceiver.getSelectionModel().getSelectedItem())) {
				throw new NullPointerException("Не може да се достави до същия офис");
			}
			if (sendToAddress && address.equals("")) {
				throw new NullPointerException("Полето адрес не може да е празно");
			}

			if (Date.valueOf(receiveDate).after(Date.valueOf(clientLocalDate))) {
				throw new NullPointerException("Не може да се изпрати, преди дата на получаване");
			}
	...
	public void registerOrder() {
		...
		if (invalidFields(price, phoneSender, phoneReceiver, address, sendToAddress, receiveDate, clientLocalDate))
			return;
		...
		try {
			...
			int id_office_sender = TableQuery.getOfficeId(officeSender.getSelectionModel().getSelectedItem()),
					id_office_recipient = TableQuery
							.getOfficeId(officeReceiver.getSelectionModel().getSelectedItem()),
